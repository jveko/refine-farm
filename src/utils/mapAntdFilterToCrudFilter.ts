...
export const mapAntdFilterToCrudFilter = (
  tableFilters: Record<string, FilterValue | (string | number | boolean) | (string | number | boolean)[] | null>,
  prevFilters: CrudFilters,
  initialFilters?: CrudFilters,
): CrudFilters => {
  const crudFilters: CrudFilters = []
  const mapInitialFilter: Record<string, CrudFilter> = (initialFilters ?? []).reduce((acc, item) => {
    const field = (item as ConditionalFilter).key || (item as LogicalFilter).field
    return { ...acc, [field]: item }
  }, {})
  Object.keys(tableFilters).map((field) => {
    const value = tableFilters[field]
    const operator =
      prevFilters.filter((i) => i.operator !== "or").find((p: any) => p.field === field)?.operator ||
