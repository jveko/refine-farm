  ...
  if (filters) {
    filters.map((filter) => {
      ...
      if ("field" in filter) {
        ...
        if (Array.isArray(value) && value.every((v: string) => v === "true" || v === "false")) {
          queryFilters.push(`${field} ${mappedOperator.replace("*", "")} ${value.join(",")}`)
          return
        } else if (typeof value === "boolean") {
          queryFilters.push(`${field} ${mappedOperator.replace("*", "")} ${value}`)
          return
        } else if (typeof value === "string") {
          queryFilters.push(`${field} ${mappedOperator} '${value}'`)
          return
        } else if (Array.isArray(value)) {
          queryFilters.push(`${field} ${mappedOperator} ${value.map((x) => `'${x}'`).join(",")}`)
          return
        }
        queryFilters.push(`${field} ${mappedOperator} ${value}`)
      }
